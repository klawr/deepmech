  ...
  detector: {
    ...
    getCrops(image, element, nodes, constraints = []) {
      ...
      blob = blob.arraySync().map((b, idx) => {
        ...
        if (info[idx].mirror == 1) return tf.reverse2d(b, 1);
        else if (info[idx].mirror == 2) return tf.reverse2d(b, 0);
        else if (info[idx].mirror == 3) return tf.reverse2d(b);
        else return b;
      });
...
export function updateNodes(nodes) {
  ...
  nodes.forEach((e) => {
    function makeUnique(id) {
      return model.nodes.find((n) => n.id === id) ? makeUnique(id + "x") : id;
    }
...
export function updateConstraints(constraints) {
