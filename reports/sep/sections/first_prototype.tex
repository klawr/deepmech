...
To create a prototype capable of detecting hand-drawn mechanisms, the following necessities have to be considered:
...
The topic of prepending work was the recognition of hand-drawn mechanical symbols \cite{Lawrence2020}.
Thereupon, the previous trained model is extended to be able to provide not only a classification, but also the location of an element in an image of arbitrary size.
The respective model can be loaded using \name{Keras'}, % TODO citation should be in the introduction.
\code{model.load\_model} and by issuing the \code{summary} method we get listing~\ref{lst:srp_model}, which is a summary of the final model for node detection.
This model is trained to detect whether an image contains a \name{node}, a \name{base-node} or neither.
...
This model has to be extended to be able to receive not only a preprocessed image, but a 
One way to localize detections in an image is to scan each sector of an image and predicting the generated crops.
...
A Jupyter notebook testing the performance of this method can be reviewed at \url{https://aka.klawr.de/sep#1}. % TODO Set this URL.
...
This would reduce the accuracy of the localization, but would increase the speed of the operation.

Another approach would be to use the properties of convolutional layers to restructure the model and thereby making the whole procedure much more efficient.

By transforming a model into a so-called Fully Convolutional Network (FCN) all dense layers are replaced by convolutional layers\footnote{The layers affected here are the input and output layer.}.
The input layer, which is also replaced by a two-dimensional convolutional layer is furthermore able to accept images of arbitrary size.
...
Besides the model to detect nodes, it is now necessary to train a model that can detect and classify constraints.
To create such a model fitting training data has to be generated first.

The model should be able to distinguish between two types of constraints.
One type of constraint prohibits nodes to change the range between each other, which allows only for rotational movement.
It is represented by an arrow with an arc at the origin.
The other type of constraint prohibits motion that would change the angle between two nodes, which only allows translational movement between two nodes.
...
Since constraints can occur at all possible angles, the data must be generated in such a way that it resembles reality the most.
