...
A prototype for the inference model has to be able to detect and to localize \name{nodes} and to detect \name{constraints}, which are connecting pairs of \name{nodes}.
Subsequently, the gathered information has to be transformed into a usable format for further processing.
Constraints limit the \code{nodes} in their degree of freedom, resulting in a mechanical linkage if they are arranged accordingly.
...
The respective model can be loaded using \name{Keras}~\cite{Chollet2019}.
\name{Keras}' \code{model.load\_model} loads the inference model and by issuing the \code{summary} method of the resulting object we get listing~\ref{lst:srp_model}.
...
A naive way to localize images in a bigger image is to scan each sector of an image and predicting the generated crops.
...
A Jupyter notebook testing the performance of this method can be viewed at \aka{https://github.com/klawr/deepmech/blob/master/reports/sep/notebooks/2.1.1-scanning_an_image.ipynb}.
...
An example of the usage of this code can be found at \aka{https://github.com/klawr/deepmech/blob/master/reports/sep/notebooks/2.1.2-fcn.ipynb}, where the intricate differences between both approaches are examined.
...
For each type approximately 1200 images were drawn, 400 for each range, resulting in 2400 images\footnote{The raw data can be downloaded at \aka{https://drive.google.com/file/d/1Xff-qFCLYnNqsBIPKw7WiMMz3qa8VIfy/view}.}.
...
Between 3 and 10 nodes are randomly placed on the image\footnote{In an intermediate step the node data from previous work is modified to consist of white symbols on black background, instead of random grayscale images. It can be accessed at \aka{https://drive.google.com/file/d/18uzwfqM4fhfdAQDBd5dvT6k-a-pUWvHR}.}.
...
The respective notebook containing the code for this process can be viewed at \aka{https://github.com/klawr/deepmech/blob/master/reports/sep/notebooks/2.2.1-constraint_data_gen.ipynb}.
...
After cropping and reshaping the training data the images can be inserted into a training process of a \name{Keras} model.
The notebook demonstrating this operation can be viewed at \aka{https://github.com/klawr/deepmech/blob/master/reports/sep/notebooks/2.2.2-connecting_nodes.ipynb}.
...
The resulting dataset contains 119.189 images, which are subsequently transformed into a \name{tensorflow record}\footnote{The data can be downloaded from \aka{https://drive.google.com/file/d/1GkEI71ogtMRSQKdO30J1DVNmVJrSVZRN/view}.}.
...
The model is created using the functional API of \name{Keras}, which has no real impact on the result, but is just another way of defining models\footnote{See \aka{https://keras.io/guides/functional_api/}.}.
...
\name{TensorBoard}~\cite{Google2021} is used to log the accuracy and the loss of the model during training.
...
\subsubsection{Combining node and constraint detection} \label{ch:combine_node_constraint_detection}
...
A pipeline of this operation can be reviewed at \aka{https://github.com/klawr/deepmech/blob/master/reports/sep/notebooks/2.2.5-combining_node_constraints.ipynb}.
...
For this \name{TensorFlow} has an implementation in JavaScript~\cite{Google2021a}.

For the conversion of the models, the \name{tensorflowjs\_converter} is used.
This program takes the model file as input and outputs a description of the model in \name{JSON} format.
The trained weights and biases are contained in \name{group1-shard1of1.bin}\footnote{Instructions on how to import a Keras model into Tensorflow.js can be viewed at \aka{https://www.tensorflow.org/js/tutorials/conversion/import_keras}.}.
...
The full file can be reviewed at \aka{https://raw.githubusercontent.com/klawr/deepmech/master/reports/sep/code/crop.js}.

For the conversion each model is assigned to a class with three members.
\code{model} contains the content of the JSON file and \code{bin} the base64-string without any further modifications.
Furthermore, objects created by this class contain a \code{load} function.
\code{load} returns the respective model by parsing the JSON into an object.
The \code{weightData} of the inference model is added by decoding the content of \code{bin}.
The inference model can be loaded by issuing \code{tf.loadLayersModel(new Crop())}, provided by the \name{Tensorflow.js} library.
...
Functions loading the inference models are implemented into the JavaScript pipeline by embedding all necessary functions into an object called \code{deepmech}.
...
\begin{enumerate}
    ...
    The returned value of this function is immediately used as input for the \code{toFullyConv} function.
    \item \code{constraintDetector} --- This function is similar to \code{nodeDetector}, but without the necessity to convert the model into a FCN.\@
    ...
    \item \code{getCrops} --- A function which uses an image and previously determined nodes to create crops. These crops are equivalent to those generated in Chapter~\ref{ch:cropping_images}.
...
By converting all the necessary steps into a JavaScript pipeline, the \code{deepmech} object may be implemented into other libraries or applications.
For demonstration purposes, it is implemented using the Node.js~\cite{Joyent2021} runtime environment, by using \name{ijavascript} and the \name{conda} package.
A Jupyter Notebook is created to show the results.
It can be viewed at \aka{https://github.com/klawr/deepmech/blob/master/reports/sep/notebooks/2.3.1-nodejs_implementation.ipynb}.

\subsection{Implementation into mec2}

\subsubsection{mec2}

\name{mec2} is a physics simulation engine written in JavaScript~\cite{Goessner2021a}\cite{Goessner2019a}\cite{Goessner2019b}.
It is designed to easily create planar mechanical linkages.
The models can be rendered on a canvas HTML element using the \name{g2} graphics library\cite{Goessner2021}.
\name{mec2} models provide respective native functionality for \name{g2}.

\name{mec2} models are defined using the JSON-format, as shown in Listing~\ref{lst:mec2_example}.
Therein \code{nodes} are described by a unique \code{id}, their location in respect to the bottom left of the view using the properties \code{x} and \code{y} and if they are considered a \code{base}\footnote{Bases are simulating the ground and are treated as being unmovable}.
Constraints have an \code{id} too.
Furthermore \code{p1} and \code{p2} have to be defined, which refer to nodes by their \code{id}.
The \code{len} and \code{ori} property have a \code{type} which, for the scope of this project, can be either \code{const} or \code{free}, defining the constrained behaviour of the respective nodes \code{p1} and \code{p2} either lateral or rotational (or both).
...
\name{mec2}s functionality is divided into multiple plugins; some of which are optional, but some are necessary to simulate functioning mechanisms.
...
At the time of writing five other modules\footnote{The other modules being \name{mec2.load}, \name{mec2.drive}, \name{mec2.view}, \name{mec2.msg.en} and \name{mec2.shape}.} exist to extend the functionality of \name{mec2}, but they do not concern the implementation of \name{deepmech} yet and are not further discussed here.

Additionally, \name{mec2} provides a custom HTML element, which allows for an easy implementation into web pages\footnote{For more information about custom HTML elements see \aka{https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_custom_elements}.}.
The object defining the model is given as JSON inside the \code{innerHTML} of the respective custom HTML to define the input, which is respectively parsed using the built-in function \code{JSON.parse()}.
...
The \name{canvasInteractor.js} is a JavaScript object and acts as an event manager for the HTML canvas element.
It is used to centralize all interactions and animation requests; e.g. the \code{canvasInteractor} raises one \code{requestAnimationFrame}.
Other components may register their events to the \code{canvasInteractor} instead of raising the respective events on their own.
This approach parallelizes all requests, because they are all handled in one function call.

New \code{instances} can be added to the \code{canvasInteractor} object by calling its \code{add} function.
To create a new instance of the \code{canvasInteractor} the \code{create} function has to be issued.
It applies the prototype of the \code{canvasInteractor} object onto a given variable.
...
The background of the drawing canvas is colored black (\code{\#000}) by issuing the \code{rec} command of \name{g2} for the active viewport\footnote{Please note that figure~\ref{fig:prototype_drawing} contains an inverted image for aesthetic reasons.}.
...
A \name{mec2} model is usually defined by more than just nodes and costraints.
Therefore all elements which are not part of \code{element.\_model.nodes} or \code{element.\_model.constraints} are filtered first; e.g.\ the chart which can be seen in figure~\ref{fig:generated_data_samples} is removed.

The \code{ply\_placeholder} is responsible for rendering user-generated drawing.
The default behavior of the \name{canvasInteractor} when pressing the pointer-button in combination with movement of the pointer is to drag the viewport.
When an element is selected by the \name{g2.selector} the selected element is dragged instead.
When the drawing mode is active this behavior has to be replaced.
If the \code{pointerdown} is issued while in drawing mode, an array is created which is then filled by the coordinates of the pointer on each \code{tick}\footnote{A \code{tick} is issued by the \name{canvasInteractor} on each \code{requestAnimationFrame}.}.
...
The resulting prediction is the used to update the \name{mec2} model.
...
The assembled \name{mec2} model can than be rendered, simulated and interacted with.
