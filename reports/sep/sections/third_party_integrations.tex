...
The \code{WebView2} component itself is controlled by the \code{DeepmechHandler} property which is a \code{DeepmechWebView} object\footnote{\aka{https://github.com/klawr/deepmech/tree/blob/master/src/wpf/deepmech/MainWindow.xaml}.}.
...
The WPF-application has a respective event-handler to process messages sent via \code{chrome.webview.postMessage}, which can be seen in Listing~\ref{lst:wpf_event_handler}.
