...
The \code{DeepmechCanvas} component has a \code{canvasRef} component which is initially set to \code{const canvasRef = React.useRef(null)}.
...
The first argument given to this function is called after the component is rendered the first time.
...
\code{dispatch} sets certain values which will be further examined in Chapter~\ref{ch:state_manipulation}.

% TODO add some text about the first function (return value etc.) Maybe put the second argument first? because the text for the first is going to be big...
