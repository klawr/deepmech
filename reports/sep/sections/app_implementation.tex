...
The usefulness of this will be examined in in Chapter~\ref{ch:undo_redo}.
...
\begin{lstlisting}[label={lst:add_remove_list_element}, caption={Adding or removing a node in \code{mecElement}.}]
const add = (up && action.idx === 'add') || (!up && action.idx === 'remove');
if (action.list === 'nodes') {
    if (add) {
        const node = { ...action.value };
        if (ref._model.nodeById(node.id)) return;
...
The excerpt in Listing~\ref{lst:add_remove_list_element} shows the current implementation to add a node element to \code{mecElement}\footnote{The \code{mecElement} is referenced by \code{ref}.}.
Three functions have to be called to register an element.
The \code{mec.node.extend} function is used to apply the respective prorotype to the \code{node} variable\footnote{This makes the object also unserializable by \name{React-Redux}.}.
The next step is to add the node to the \code{mecElement.\_model}.
The third function initializes the node. % TODO and this does exactly what?

To remove a node the \code{mecElement.\_model.removeNode} is called.

Before the model can be rendered, another thing has to be issued.
...
Because of this, the respective references have to be created again.
