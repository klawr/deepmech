...
def decode(example):
    ...
    def intersection(box1, box2):
        ...
        x2 = tf.maximum(box1[2], box2[2])
        y2 = tf.maximum(box1[3], box2[3])
    ...
    for key in feature_description:
        ...
        # Get width and height OR angle and length
        _d1 = tf.divide(f[3], 36)
        tmp = tf.cast(tf.cond(f[0] < 2, lambda: 36, lambda: 360), tf.int64)
        _d2 = tf.divide(f[4], tmp)
        d1.append(_d1)
        d2.append(_d2)

        # b and h are either calculated through actual width and height
        # or using the sin/cos of w times l
        tmp = tf.cast((tf.acos(0.)/90), tf.float64)
        b, h = tf.cond(f[0] < 2,
            lambda: [_d1, _d2],
            lambda: [_d1 * tf.cos(_d2)*tmp, _d1 * tf.sin(_d2)*tmp])

        # Add objectness score
        box1 = tf.concat((cell - (b / 2), cell + (h / 2)), axis = 0)
        box2 = tf.concat((cell, cell + 1), axis = 0)
...
log_dir = path.join('logs', 'sep_yolo2', datetime.now().strftime("%Y-%m-%dT%H-%M-%S"))
model_path = path.join('models', 'devel', 'sep_yolo2.h5')
...
def get_bounding_boxes(pred):
    ...
    indices = tf.where(tf.maximum(lists[4] - 0.5, 0))
    num = indices.shape[0]
    ...
    resize = tf.broadcast_to([36, 36, 36, 36, 1, 1, 1, 1, 1], [num, 9])
    resize = tf.cast(resize, tf.float64)

    results = results * resize

    def bounding_box(result, b, h):
        x1 = result[0] - b
        y1 = result[1] - h
        x2 = result[0] + b
        y2 = result[1] + h
    ...
    def bounding_node(result):
        b, h = result[2] / 2, result[3] / 2

        return bounding_box(result, b, h)

    def bounding_constraint(result):
        w = result[3] * tf.cast(tf.acos(0.) / 9, tf.float64)
        b = tf.abs(result[2] * tf.cos(w) / 2)
        h = tf.abs(result[2] * tf.sin(w) / 2)
    ...
    bases = tf.map_fn(bounding_node, bases)
    nodes = tf.map_fn(bounding_node, nodes)
    rot   = tf.map_fn(bounding_constraint, rot)
    trans = tf.map_fn(bounding_constraint, trans)

    return bases, nodes, rot,trans

# a = next(iter(take_data(record_path, 1)))
# pred = decode(a)
