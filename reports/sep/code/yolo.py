...
record_path = path.join('data', 'sep_processed_01', 'train.tfrecord')
...
def decode(example):
    ...
    for key in feature_description:
        ...
        b, h = tf.split(tf.divide(tf.stack((f[3], f[4]), axis=-1), 36), 2, axis=-1)
...
log_dir = path.join('logs', 'sep_yolo', datetime.now().strftime("%Y-%m-%dT%H-%M-%S"))
model_path = path.join('models', 'devel', 'sep_yolo.h5')
...
def get_bounding_boxes(pred):
    ...
    resize = tf.broadcast_to([36, 36, 36, 36, 1, 1, 1], [num, 7])
