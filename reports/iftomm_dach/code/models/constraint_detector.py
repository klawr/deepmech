...
processed = path.join('data', 'iftomm_dach_processed_02')
...
def decode_record(record_path, feature_description, shape = (96, 96, 1),
...
model.add(tf.keras.layers.Conv2D(16, (4,4), activation='relu', padding='same', input_shape=(96, 96, 1)))
...
log_dir = path.join('logs', 'iftomm_dach_constraint_detector', datetime.now().strftime("%Y-%m-%dT%H-%M-%S"))
model_path = path.join('models', 'devel', 'iftomm_dach_constraint_detector.h5')
